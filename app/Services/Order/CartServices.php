...
class CartServices extends BaseServices
{
    ...
    public function getValidCartList($userId)
    {
        ...
        $goodIds        = $list->pluck('goods_id')->toArray();
    ...
    {
        return Cart::query()->where('user_id', $userId)->where('goods_id', $goodsId)->where('product_id', $productId)->first();
    }
     ...
     * @return array<\Illuminate\Database\Eloquent\Builder<Goods>|\Illuminate\Database\Eloquent\Builder<GoodsProduct>> | void
    ...
    {
        return Cart::query()->where('user_id', $userId)->whereIn('product_id', $productIds)->update(['checked' => $isChecked]);
    }
    ...
    public function clearCartGoods($userId, $cardId = null)
    {
        if (empty($cardId)) {
            return Cart::query()->where('user_id', $userId)->where('checked', 1)->delete();
        }
        return Cart::query()->where('user_id', $userId)->where('id', $cardId)->delete();
    }
}
