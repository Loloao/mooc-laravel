...
class GrouponServices extends BaseServices
{
    ...
    {
        return GrouponRules::whereStatus(GrouponEnums::RULE_STATUS_ON)->orderBy($page->sort, $page->order)->paginate($page->limit, $columns, 'page', $page->page);
    }
    ...
    public function isOpenOrJoin($userId, $grouponId)
    {
        return Groupon::query()->whereUserId($userId)->where(function (Builder $builder) use ($grouponId) {
            return $builder->where('id', $grouponId);
    ...
    {
        return Groupon::query()->whereGrouponId($openGrouponId)->where("status", '!=', GrouponEnums::STATUS_NONE)->count('id');
    }
    ...
    public function checkGrouponValid($userId, $ruleId, $linkId = null)
    {
        ...
        if (is_null($rule)) {
            $this->throwBusinessException(CodeResponse: CodeResponse::PARAM_VALUE_ILLEGAL);
        }
    ...
    {
        $shareUrl = 'http:://' . $rules->goods_id;
        $qrCode   = QrCode::format('png')->margin(1)->size(290)->generate($shareUrl);

        $manager    = new ImageManager(new Driver());
        $goodsImage = $manager->read(file_get_contents($rules->pic_url))->resize(660, 660);
        $image      = $manager
            ->read(resource_path('image/back_groupon.png'))
            ->place($qrCode, 'top-left', 460, 770)->place($goodsImage, 'top-left', 71, 69)
            ->text($rules->goods_name, 65, 867, function (FontFactory $font) {
                $font->color('rgb(167, 136, 69)');
                $font->size(28);
                $font->file(resource_path('ttf/msyh.ttf'));
            });
        $fileName = 'groupon/' . Carbon::now()->toDateString() . '/' . Str::random() . '.png';
