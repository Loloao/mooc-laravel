...
class BooleanSoftDeleteTest extends TestCase
{
    ...
    public function testDeleteByBuilder()
    {
        ...
        $res = Goods::query()->whereId($this->goodsId)->delete();
        ...
        $res = Goods::withTrashed()->whereId($this->goodsId)->restore();
        $this->assertEquals(1, $res);
        $goods = Goods::onlyTrashed()->whereId($this->goodsId)->first();
        $this->assertNull($goods);
        $goods = Goods::query()->whereId($this->goodsId)->first();
        ...
        $res = Goods::withTrashed()->whereId($this->goodsId)->forceDelete();
        $this->assertEquals(1, $res);
        $goods = Goods::query()->whereId($this->goodsId)->first();
        $this->assertNull($goods);
        $goods = Goods::onlyTrashed()->whereId($this->goodsId)->first();
